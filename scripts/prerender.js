...
async function prerender() {
  ...
  const runtimePath = path.resolve("scripts/runtime.js");
  ...
  try {
    ...
    const env = {
      VITE_SUPABASE_URL:
        ...
        "https://kcqpkuzmejhdbjflxhct.supabase.co",
      VITE_SUPABASE_PUBLISHABLE_KEY:
        process.env.VITE_SUPABASE_PUBLISHABLE_KEY ||
        "sb_publishable_Aa1As_5NSzXZGTjktLUhFg_uJC7z2vP",
    };
    const ctx = {
      waitUntil: () => {},
    ...
    if (response.status === 200 && html && html.includes("<html")) {
      ...
      // Read interactive runtime script
      let runtimeScriptCode = "";
      if (fs.existsSync(runtimePath)) {
        runtimeScriptCode = fs.readFileSync(runtimePath, "utf-8");
      }

      const styleTag = inlineCss
        ? '<style id="inlined-styles">' + inlineCss + "</style>"
        : "";
      const baseTag = '<base href="/portfolio/" />' + styleTag;
      ...
      console.log(
        "Successfully prerendered index.html and 404.html with inlined CSS, interactive runtime, and /portfolio/ base for GitHub Pages",
      );
    } else {
      console.warn(
        "SSR returned non-200 response:",
        response.status,
        html.slice(0, 200),
      );
    }
